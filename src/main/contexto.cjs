...
function textoSkill(rutaCatalogo) {
  return [
    '---',
    'name: ' + NOMBRE_SKILL,
    'description: Traer a esta sesion contexto guardado de otros proyectos o sesiones anteriores: memorias de Claude Code, CLAUDE.md de otros repos, y los grafos de codigo indexados. Tambien importa paquetes de contexto armados con Cockpit (carpetas con manifest.json). Usar cuando el usuario pida traer contexto, recordar lo de otro proyecto, seguir algo de otra sesion, importar un paquete, o escriba /cockpit-memory.',
    '---',
    '',
    ...
    '# Importar un paquete',
    '',
    'Si el usuario escribe `/cockpit-memory importar <ruta>`, o te pasa una carpeta',
    'que tiene adentro un `manifest.json` generado por Cockpit, eso es un paquete de',
    'contexto: alguien eligio a mano que llevarse y lo dejo ahi.',
    '',
    'Un paquete se ve asi:',
    '',
    '    manifest.json          que hay y donde va cada cosa',
    '    LEEME.md               lo mismo pero para una persona',
    '    memorias/<proyecto>/   archivos .md',
    '    claude-md/<proyecto>/  CLAUDE.md de cada repo',
    '    conversaciones/        <sesion>.resumen.md y a veces <sesion>.jsonl',
    '    grafos/                solo si el paquete es autocontenido',
    '',
    '## Que hacer',
    '',
    '1. Lee el `manifest.json`. **No leas los archivos todavia**: un transcript',
    '   completo puede pesar megabytes y no entra en contexto.',
    '',
    '2. Mostrale al usuario que trae el paquete, en una tabla corta agrupada por',
    '   proyecto, y de donde salio (`maquinaOrigen`, `generadoEn`).',
    '',
    '3. Preguntale que quiere hacer con cada cosa. **No copies nada sin',
    '   confirmacion**: importar pisa archivos que Claude Code lee en cada sesion.',
    '',
    '4. Las cuatro cosas NO se importan igual:',
    '',
    '   - **Memorias**: si solo las quiere leer, Read del campo `archivo` y las',
    '     resumis. Si las quiere instalar, copialas al campo `destino` de cada una',
    '     y **agrega su linea a `MEMORY.md`** de esa carpeta: sin eso los archivos',
    '     existen pero Claude Code no los encuentra.',
    '',
    '   - **CLAUDE.md**: va a `destino`. Casi siempre ya hay uno ahi. Compara los',
    '     dos y ofrece fusionar antes de pisar; nunca sobreescribas de una.',
    '',
    '   - **Conversaciones**: lo util es el `.resumen.md`, que es chico y tiene los',
    '     mensajes del usuario en orden. Leelo. El `.jsonl` (si esta) es el',
    '     transcript entero: **no lo leas para resumirlo**, son megabytes.',
    '',
    '   - **Grafos**: el paquete trae el puntero, no el grafo. Lo correcto casi',
    '     siempre es reindexar en esta maquina con lo que dice `reindexar`, no',
    '     copiar el indice viejo. Si `desactualizado` es true, mas todavia.',
    '',
    '## El detalle que rompe todo si se ignora',
    '',
    'El nombre de la carpeta de un proyecto (`projectDir`) es la ruta del repo con',
    'los separadores reemplazados. Es de la maquina que exporto.',
    '',
    'Antes de copiar cualquier cosa, fijate si `~/.claude/projects/<projectDir>/`',
    'existe **en esta maquina**. Si no existe, el repo esta en otra ruta o todavia',
    'no se abrio nunca: preguntale al usuario a que proyecto local corresponde en',
    'vez de crear una carpeta con el nombre de la maquina vieja, que quedaria',
    'huerfana y no la leeria nadie.',
    '',
    'Lo mismo con un `.jsonl`: solo se puede retomar con `claude --resume` si queda',
    'en la carpeta que corresponde al cwd de ESTA maquina. El manifiesto trae',
    '`retomableSi` con la ruta original; si no coincide, decilo en vez de prometer',
    'que va a andar.',
    '',
  ].join('\n');
}
